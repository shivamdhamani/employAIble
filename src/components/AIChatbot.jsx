...
import {
  ...
  Shield, Landmark, Briefcase, Award, Zap, Settings, Key,
  AlertCircle
} from 'lucide-react'

// Pre-packaged domain knowledge engine for employAIble fallback
...
// Call live Google Gemini 1.5 Flash REST API
async function callLiveGeminiAPI(apiKey, query) {
  const SYSTEM_PROMPT = `You are Sahayak AI (सहायक), the official AI Career & Government Welfare Assistant for employAIble, an inclusive employment platform for Persons with Disabilities (PwD) in India under the Rights of Persons with Disabilities (RPwD) Act 2016.
Key knowledge:
- SPP (Sustainable Placement Probability) algorithm: 5 dimensions (30% Skills, 20% Transit, 20% Physical Access, 15% Assistive Tech, 15% Shift Compatibility) to predict 90-day retention.
- Government Schemes: ADIP scheme (free assistive devices, up to ₹25,000 motorized tricycle subsidy via ARJUN portal adip.disabilityaffairs.gov.in), Swavlamban Health Insurance (₹2 Lakh cover, free for income <= 3L/yr), NIRAMAYA (₹1 Lakh cover for autism/intellectual/cerebral palsy via National Trust), NHFDC self-employment loans up to ₹5 Lakh, 4% quota under Section 34 of RPwD Act 2016.
- Employer Subsidies: Up to 70% reimbursement for workplace adaptations under Accessible India Scheme, EPFO/PF contribution waiver for PwD employees earning up to ₹25k/mo.
- Keep responses friendly, encouraging, concise, and structured with bullet points. Suggest official government portals where helpful. Support English or Hindi.`
  ...
  if (!replyText) throw new Error('No response text generated by Gemini')
  return replyText
}

// Fallback domain-specific matcher
...
export default function AIChatbot() {
  ...
  const [apiKey, setApiKey] = useState(() => {
    return localStorage.getItem('sahayak_gemini_key') || import.meta.env.VITE_GEMINI_API_KEY || ''
  })
  ...
  const isLiveConnected = Boolean(apiKey && apiKey.trim().length > 10)
  ...
  const handleSaveApiKey = () => {
    ...
    setApiKey(clean)
    if (clean) {
      localStorage.setItem('sahayak_gemini_key', clean)
    } else {
      localStorage.removeItem('sahayak_gemini_key')
    }
  ...
  const handleSend = async (textToSend) => {
    ...
    // Check if live Gemini API is connected
    if (isLiveConnected) {
      try {
        const liveReply = await callLiveGeminiAPI(apiKey, query)
        const botMsg = {
          id: Date.now() + 1,
          sender: 'bot',
          isLiveAI: true,
          text: liveReply,
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          chips: ['Tell me more', 'What schemes match this?', 'How does SPP calculate this?']
        }
        setMessages(prev => [...prev, botMsg])
        setIsTyping(false)
        if (voiceEnabled) speakText(liveReply)
        return
      } catch (err) {
        console.warn('Live Gemini API error, falling back to local domain engine:', err)
      }
    }

    // Fallback domain-specific engine
    setTimeout(() => {
      ...
      {!open && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999 }}>
          <button
            ...
            style={{
              ...
              background: isLiveConnected
                ? 'linear-gradient(135deg, #15803D 0%, #0E7490 100%)'
                : 'linear-gradient(135deg, #0056B3 0%, #0E7490 100%)',
              color: '#FFFFFF',
              border: 'none',
              cursor: 'pointer',
          ...
          >
            ...
            <span style={{
              fontSize: 10,
              background: isLiveConnected ? '#DCFCE7' : '#E0F5F8',
              color: isLiveConnected ? '#15803D' : '#0E7490',
              padding: '2px 7px',
              borderRadius: 12,
              fontWeight: 800,
              letterSpacing: '.04em',
              ...
              {isLiveConnected ? <><Zap size={10} /> LIVE API</> : 'ONLINE'}
            </span>
            {unread > 0 && (
              <span style={{
                position: 'absolute',
      ...
      {open && (
        <div
          style={{
            ...
            border: isLiveConnected ? '1.5px solid #86EFAC' : '1.5px solid #BFDBFE',
            display: 'flex',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{
                    fontSize: 10,
                    background: isLiveConnected ? '#DCFCE7' : 'rgba(220, 252, 231, 0.25)',
                    color: isLiveConnected ? '#15803D' : '#86EFAC',
                    ...
                    {isLiveConnected ? <><Zap size={10} /> Gemini 1.5 Live</> : 'Domain AI'}
                  </span>
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <button
                ...
                style={{
                  background: showSettings ? '#FFFFFF' : isLiveConnected ? '#15803D' : 'rgba(255, 255, 255, 0.15)',
                  border: 'none',
                  borderRadius: 6,
                  width: 30,
                  height: 30,
                  ...
                  justifyContent: 'center',
                  cursor: 'pointer',
                  color: showSettings ? '#0056B3' : '#FFFFFF',
                  transition: 'all .15s'
                }}
              >
                <Settings size={15} />
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontWeight: 700, color: '#1E293B', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <Key size={14} color="#0056B3" /> Live Google Gemini API Key
                </span>
                <span style={{
                  fontSize: 11,
                  fontWeight: 700,
                  color: isLiveConnected ? '#15803D' : '#64748B'
                }}>
                  {isLiveConnected ? '✓ Connected' : 'Optional (Default: In-Browser)'}
                </span>
              </div>
              ...
              <div style={{ marginTop: 6, fontSize: 11, color: '#64748B', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <a
                  href="https://aistudio.google.com/app/apikey"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: '#0056B3', textDecoration: 'none', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 3 }}
                >
                  Get free key from Google AI Studio <ExternalLink size={10} />
                </a>
                {isLiveConnected && (
                  <button
                    type="button"
                    ...
                    style={{ background: 'none', border: 'none', color: '#B91C1C', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}
                  >
                    Disconnect Key
                  </button>
                )}
          ...
          {voiceEnabled && (
            <div style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <Volume2 size={13} /> Voice reading is ON (speaking answers aloud)
              </span>
              <button
                type="button"
                onClick={() => {
          ...
          <div style={{
                    ...
                    {msg.sender === 'bot' && (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, paddingTop: 6, borderTop: '1px solid #F1F5F9' }}>
                        <span style={{
                          fontSize: 10,
                          fontWeight: 700,
                          color: msg.isLiveAI ? '#15803D' : '#0056B3',
                          background: msg.isLiveAI ? '#DCFCE7' : '#E8F0FA',
                          padding: '1px 6px',
                          borderRadius: 8,
                          display: 'flex',
                          alignItems: 'center',
                          ...
                          {msg.isLiveAI ? <><Zap size={10} /> Live Gemini AI</> : 'RPwD Knowledge Base'}
                        </span>
                        <button
                          type="button"
            ...
            {isTyping && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{
                  width: 28, height: 28, borderRadius: '50%', background: isLiveConnected ? '#15803D' : '#0056B3',
                ...
                <div style={{
                  ...
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: isLiveConnected ? '#15803D' : '#0056B3', animation: 'pulse 1s infinite' }} />
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: isLiveConnected ? '#15803D' : '#0056B3', animation: 'pulse 1s infinite 0.2s' }} />
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: isLiveConnected ? '#15803D' : '#0056B3', animation: 'pulse 1s infinite 0.4s' }} />
                  <span style={{ fontSize: 11.5, color: '#64748B', marginLeft: 4, fontWeight: 600 }}>
                    {isLiveConnected ? 'Querying Google Gemini 1.5 Flash API...' : 'Analyzing RPwD standards...'}
                  </span>
                </div>
              </div>
            )}
